...
def test_v2_0_0_deprecated_gpus(mock_device_count, mock_is_available, tmpdir):
    with pytest.deprecated_call(match=r"is deprecated in v1.6 and will be removed in v2.0."):
...
def test_v2_0_0_deprecated_tpu_cores(mock_device_count, mock_is_available, tmpdir, monkeypatch):
    monkeypatch.setattr(pytorch_lightning.utilities.device_parser, "_TPU_AVAILABLE", True)
    with pytest.deprecated_call(match=r"is deprecated in v1.6 and will be removed in v2.0."):
        _ = Trainer(default_root_dir=tmpdir, tpu_cores=8)


@mock.patch("pytorch_lightning.accelerators.ipu.IPUAccelerator.is_available", return_value=True)
def test_v2_0_0_deprecated_ipus(mock_is_available, tmpdir, monkeypatch):
